...
class Config:
    ...
    # === モデル設定: EfficientNet-B4 ===
    # B5ではなくB4 (380px) を使用
    model_name = "tf_efficientnet_b4_ns"
    img_size = 380
    
    # GPU 2枚なら32でもOK（もしOOMなら16へ）
    batch_size = 32
...
if __name__ == '__main__':
    ...
    # --- Transforms (ご指定の内容に変更) ---
    train_transforms = transforms.Compose([
        transforms.Resize((Config.img_size, Config.img_size)),
        
        # 1. Horizontal Flip (水平反転のみ)
        transforms.RandomHorizontalFlip(p=0.5),
        
        # 2. Random Rotate (回転)
        transforms.RandomRotation(degrees=15),
        
        # 3. Random Contrast & Brightness
    ...
    # --- Training Loop ---
    best_auc = 0.0
    weight_dir = os.path.join(Config.output_dir, 'Weight_B4')
    prediction_dir = os.path.join(Config.output_dir, 'Prediction_B4')
    os.makedirs(weight_dir, exist_ok=True)
    os.makedirs(prediction_dir, exist_ok=True)

    save_path = os.path.join(weight_dir, 'best_model_b4_2gpu.pth')
    ...
    df = pd.DataFrame(predictions, columns=['image_id', 'label'])
    submit_path = os.path.join(Config.output_dir, 'Prediction_B4', 'submission_ensemble_final_b4.csv')
